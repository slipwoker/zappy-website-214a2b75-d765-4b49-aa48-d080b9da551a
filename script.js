document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function validateField(field) {
      ...
      if (required && value === '') {
        showError(field, customMessage || field.getAttribute('data-required-message') || 'This field is required.');
        return false;
      }
      ...
      if (type === 'email' && !isValidEmail(value)) {
        showError(field, customMessage || 'Please enter a valid email address.');
        return false;
      }

      if (type === 'tel' && !isValidPhone(value)) {
        showError(field, customMessage || 'Please enter a valid phone number.');
        return false;
      }

      if (minLength > 0 && value.length < minLength) {
        showError(field, customMessage || 'Please enter at least ' + minLength + ' characters.');
        return false;
      }

      if (maxLength > 0 && value.length > maxLength) {
        showError(field, customMessage || 'Please enter no more than ' + maxLength + ' characters.');
        return false;
      }
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      fetch(action, {
        ...
        .catch(function () {
          ...
          const errorMsg = contactForm.querySelector('.form-error-message, [data-form-error-message]');
          if (errorMsg) {
  ...
  const ANIMATE_THRESHOLD = 0.15;

  const animatableElements = document.querySelectorAll(
    '[data-animate], .animate-on-scroll, .fade-in, .slide-up, .slide-in'
  );
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "מכבסת טיפ טופ משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '214a2b75-d765-4b49-aa48-d080b9da551a',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '214a2b75-d765-4b49-aa48-d080b9da551a';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '214a2b75-d765-4b49-aa48-d080b9da551a';
